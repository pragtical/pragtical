...
static SDL_Surface *font_allocate_glyph_surface(RenFont *font, FT_GlyphSlot slot, int bitmap_idx, GlyphMetric *metric) {
  ...
  if (atlas_idx < 0) {
    ...
    for (int i = 0; i < FONT_BITMAP_COUNT(font); i++) {
      font->glyphs.atlas[i] = check_alloc(SDL_realloc(font->glyphs.atlas[i], sizeof(GlyphAtlas) * (font->glyphs.natlas + 1)));
  ...
  if (surface_idx < 0) {
    ...
    switch(font->antialiasing) {
      case FONT_ANTIALIASING_SUBPIXEL:
        format = SDL_PIXELFORMAT_RGB24; break;
      case FONT_ANTIALIASING_GRAYSCALE:
        format = SDL_PIXELFORMAT_INDEX8;
      default:
        format = SDL_PIXELFORMAT_RGBA32; break;
    }
...
static void font_load_glyph(RenFont *font, unsigned int glyph_id) {
  ...
  for (int bitmap_idx = 0; bitmap_idx < bitmaps; bitmap_idx++) {
    ...
    if (!font->glyphs.metrics[bitmap_idx][row]) {
      font->glyphs.metrics[bitmap_idx][row] = check_alloc(SDL_calloc(sizeof(GlyphMetric), GLYPHMAP_COL));
