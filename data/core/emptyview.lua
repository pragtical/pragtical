...
function EmptyView:update()
  ...
  if self.prev_size.x ~=  self.size.x or self.prev_size.y ~= self.size.y then
    ...
    if buttons_w < self.size.x then
      self[buttons[1].name]:set_position(
        (self.size.x / 2) - buttons_w / 2, style.padding.y
      )
    else
      self[buttons[1].name]:set_position(0, style.padding.y)
    end

    -- reposition remaining buttons
    local y = self[buttons[1].name].position.y
    for i=2, #buttons do
