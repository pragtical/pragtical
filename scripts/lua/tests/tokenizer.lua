...
test.describe("tokenizer", function()
  ...
  test.test("matches native and lua token splits for multi-capture punctuation-heavy lines", function()
    local syntax = {
      patterns = {
        {
          pattern = "warning()%s+()%(disable()%s+():()%s+()%d+()%)",
          type = {
            "keyword",
            "normal",
            "function",
    ...
    local text = "warning (disable : 4001)"
    local original = tokenizer.is_using_native()

    tokenizer.set_use_native(false)
    local lua_tokens, lua_state = tokenizer.tokenize(syntax, text, string.char(0))

    tokenizer.set_use_native(true)
    local native_tokens, native_state = tokenizer.tokenize(syntax, text, string.char(0))

    tokenizer.set_use_native(original)

    test.equal(lua_state, string.char(0))
    test.equal(native_state, string.char(0))
    test.same(lua_tokens, {
      "keyword", "warning",
      "function", " (disable",
      "operator", " :",
      "number", " 4001",
    ...
    test.same(native_tokens, lua_tokens)
  end)

  test.test("rejects late matches when a Lua pattern must start at the current position", function()
    local syntax = {
      patterns = {
        { pattern = "%f[#]#%s*[%a_][%w_]*", type = "keyword" },
        { regex = [[(?:\d[\d']*)]].."\\.?"..[[(?:\d[\d']*)]].."?"
            .."(?:[Ee][-+]?"..[[(?:\d[\d']*)]]..")?".."[fFlL]?", type = "number" },
    ...
    local text = "#pragma warning (disable : 4001)"
    local original = tokenizer.is_using_native()
