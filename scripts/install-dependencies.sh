...
main() {
  ...
  if [[ "$OSTYPE" == "linux"* ]]; then
    sudo apt-get install -qq libfuse2 ninja-build wayland-protocols libsdl2-dev libfreetype6 libharfbuzz-dev llvm-14 zsync
    pip3 install meson
    ...
    pacman --noconfirm -S \
      ${MINGW_PACKAGE_PREFIX}-{ca-certificates,gcc,python-pip,ninja,cmake,ntldd,pkg-config,mesa,freetype,harfbuzz,pcre2,sdl3} unzip
    python -m ensurepip
    pip3 install meson
  fi
}
